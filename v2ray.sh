...
TAG=$(wget --no-check-certificate -qO- https://api.github.com/repos/${REPO}/releases/latest | grep 'tag_name' | cut -d\" -f4)
if [ -z "${TAG}" ]; then
...
V2RAY_FILE="Xray-linux-${ARCH}.zip"
...
wget -O ${DOWNLOAD_PATH}/v2ray.zip https://github.com/${REPO}/releases/download/${TAG}/${V2RAY_FILE} >/dev/null 2>&1
wget -O ${DOWNLOAD_PATH}/v2ray.zip.dgst https://github.com/${REPO}/releases/download/${TAG}/${DGST_FILE} >/dev/null 2>&1
